...
for i in range(num):
  for j in range(num):
    #rand = random.random()
    rand=(0.7 - 0) * np.random.rand() 
    w[i][j] = rand
    w[j][i] = rand

omega=[2.8 for i in range(num) ]
...
"""
def Kuramoto(phi_2_number,acc):
    delta_t = 0.1/4
    phi_next = [0 for x in range(num)]
    acc = (acc-100)*0.05
    phi_2 = []

    phi_2_number = list(map(int, phi_2_number))
    ...
    for i in range(num):
        sum_sin = 0
        for j in range(num):
            sum_sin +=w[i][j]*math.sin(phi_2[j]-phi_2[i])
        if i!=0:
             phi_next[i] = phi_2[i]+(omega[i]+sum_sin/num+math.sin(phi_2[i])*acc)*delta_t
             phi_next[i] = phi_next[i] % (2*math.pi)
        else: 
             phi_next[i] = phi_2[i]+(omega[i]+sum_sin/num+math.sin(phi_2[i])*acc)*delta_t
             phi_next[i] = phi_next[i] % (2*math.pi)
    ...
    a = np.histogram(phi_2, bins=num_2,range=(0,2*math.pi))
...
class PointOnLine(gym.core.Env):
    def __init__(self):
        self.action_space=gym.spaces.Discrete(200)#行動空間wは上下
    ...
    def step(self,action):
        ...
        global number_of_step,r_li,phi_3,phi_first
        number_of_step+=1
        dt=0.01
        #acc=action-1 #もしw上なら+になる
        phi_number = self.state
        self.state = Kuramoto(phi_number,action)
        """
        ...
        self.state = list(map(int, self.state))
        for k, phi_num in enumerate(self.state):
            phi_3.extend([phi_real[k] for i in range(phi_num)])
        ...
        """
        if abs(r_li[number_of_step]-R_0)<=0.09:
            reward = -5*abs(r_li[number_of_step]-R_0)+1.0
        else:
            reward = -3.75*abs(r_li[number_of_step]-R_0)+0.3375
        
        flag = 1    
        if number_of_step>=6:
            for i in range(6):
                if abs(r_li[number_of_step-i]-R_0)>=0.08:
                    flag=0
        else :
            flag=0
    ...
    def reset(self):
        global number_of_step,r_li,w,phi_3,phi_re,phi_first
